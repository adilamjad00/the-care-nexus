...
import { useChatWithDoctorAIMutation } from '@/store/api/aiApi';
...
const DISCLAIMER =
  "I'm your clinical AI assistant. I can help with drug information, treatment protocols, differential diagnoses, and clinical decision support. I support your judgment — not replace it.";

const SUGGESTED_PROMPTS = [
  'What are the first-line treatments for Type 2 diabetes?',
  'Drug interactions for warfarin and aspirin?',
  'Differential diagnosis for acute chest pain',
  'Dosing guide for amoxicillin in adults',
];
...
export default function DoctorAIAssistantPage() {
  ...
  const [chatWithDoctorAI, { isLoading }] = useChatWithDoctorAIMutation();

  useEffect(() => {
    const saved = localStorage.getItem('carenexus_lang') || 'en';
    setLanguage(saved);
  }, []);
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      const result = await chatWithDoctorAI({
        message: text,
        conversationHistory,
        language,
      }).unwrap();
  ...
  return (
    <div className='flex flex-col h-[calc(100vh-8rem)] animate-in'>
      ...
      <div className='flex items-start justify-between mb-5'>
        <div>
          ...
          <p className='text-xs text-text-secondary ml-9'>
            Drug info · Treatment protocols · Differential diagnosis · Clinical
            support
          </p>
        </div>
        <button
      ...
      <div className='flex-1 flex flex-col rounded-2xl overflow-hidden border border-black/6 bg-white shadow-sm min-h-0 lens-card'>
        {/* Messages */}
        <div
          ref={scrollAreaRef}
        ...
        >
          ...
          {messages.map((msg) => (
            ...
            >
              ...
              <div
                className={cn(
                  'max-w-[85%] sm:max-w-[75%] px-4 py-3 rounded-3xl text-sm leading-relaxed ',
                  msg.role === 'user'
            ...
            <div className='flex-1 relative'>
              <textarea
                ...
                placeholder={
                  isRecording
                    ? 'Listening...'
                    : 'Ask about drugs, diagnosis, protocols...'
                }
                rows={1}
